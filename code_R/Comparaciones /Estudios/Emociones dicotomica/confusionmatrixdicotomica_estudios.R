...
matrizGradDico <-table(IA$IA,predGraduados)
matrizPostgradDico <-table(IA$IA, predPostgraduados)

# Calcular métricas Graduados

precision_Grad<-c(Precision(IA$IA, predGraduados, positive=0),Precision(IA$IA, predGraduados, positive=1))
recall_Grad <-c(Recall(IA$IA, predGraduados, positive=0), Recall(IA$IA, predGraduados, positive=1))
f1_score_vector_Grad<- c(F1_Score(IA$IA, predGraduados, positive=0),F1_Score(IA$IA, predGraduados, positive=1))

metrica_Grad_dico <-data.frame(Columna1=precision_Grad, Columna2=recall_Grad, Columna3=f1_score_vector_Grad)
colnames(metrica_Grad_dico) <- c("Precisión","Recall","F1_Score") 

# Calcular métricas Postgraduados.

precision_Post<-c(Precision(IA$IA, predPostgraduados, positive=0),Precision(IA$IA, predPostgraduados, positive=1))
recall_Post <-c(Recall(IA$IA, predPostgraduados, positive=0), Recall(IA$IA, predPostgraduados, positive=1))
f1_score_vector_Post<- c(F1_Score(IA$IA, predPostgraduados, positive=0),F1_Score(IA$IA, predPostgraduados, positive=1))

metrica_Post_dico <-data.frame(Columna1=precision_Post, Columna2=recall_Post, Columna3=f1_score_vector_Post)
colnames(metrica_Post_dico) <- c("Precisión","Recall","F1_Score") 
...
View(metrica_Post_dico)
> View(metrica_Grad_dico)